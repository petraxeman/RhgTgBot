...
def test_add_user(admin):
    headers = {"app-auth-token": "test_secret"}
    payload = {"initiator": ["internal", "admin"], "target": ["internal", "user"], "username": "regular-user"}
    response = fclient.post("/api/admin/add-user", headers=headers, json = payload)
    assert response.status_code == 200


def test_del_user(admin, regular):
    headers = {"app-auth-token": "test_secret"}
    payload = {"initiator": ["internal", "admin"], "target": ["internal", "user"]}
    response = fclient.post("/api/admin/del-user", headers=headers, json = payload)
    assert response.status_code == 200


def test_get_user(admin, regular):
    headers = {"app-auth-token": "test_secret"}
    payload = {"initiator": ["internal", "admin"], "target": ["internal", "user"]}
    response = fclient.post("/api/admin/get-user", headers=headers, json = payload)
    assert response.status_code == 200


def test_get_users_list(admin, regular):
    headers = {"app-auth-token": "test_secret"}
    payload = {"initiator": ["internal", "admin"], "page": 1, "page_size": 10}
    response = fclient.post("/api/admin/get-users-list", headers=headers, json = payload)
    assert response.status_code == 200
